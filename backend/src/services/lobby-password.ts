...
export async function verifyLobbyPassword(lobbyId: string, submitted: string): Promise<boolean> {
  const result = await query<{ password_hash: string | null; is_private: boolean }>(
    `SELECT password_hash, is_private FROM lobbies WHERE id = $1`,
    [lobbyId],
  );
  const row = result.rows[0];
  if (!row) return false;
  // Public lobby with no password set → always allow.
  if (!row.is_private || !row.password_hash) return true;
