...
export async function recordGameResult(input: {
  ...
  for (const p of players.rows) {
    ...
    if (won) delta = p.role === "imposter" ? 60 : 40;
    else if (ejected && p.role === "imposter") delta = 5; // caught
    else if (ejected && p.role === "civilian") delta = 2; // wrongly ejected
    ...
    const rolling = await query<{ id: string; score: number; record: string }>(
      `SELECT id, score, record FROM leaderboard_entries WHERE username = $1 AND category = $2 LIMIT 1`,
      [p.name, categorySlug],
    );
    if (rolling.rows[0]) {
      ...
      await query(
        `UPDATE leaderboard_entries SET score = $1, record = $2 WHERE id = $3`,
        [next, `${wins}W-${losses}L`, rolling.rows[0].id],
      );
    } else {
      await query(
        `
...
export async function getCategoryLeaderboard(categorySlug: string, limit = 20) {
