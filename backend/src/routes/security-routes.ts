...
export function registerSecurityRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    "/api/sessions/:sessionId/security-scan",
    {
      schema: {
        body: {
          type: "object",
    ...
    async (request, reply) => {
      ...
      if (info.ejected) return reply.code(403).send({ message: "Ejected players cannot scan." });

      const sessionRow = await query<{ editor_content: string; language: string }>(
        `
      ...
      await query(
        `INSERT INTO session_security_scans (id, session_id, player_id, passed, badge, report)
         VALUES ($1, $2, $3, $4, $5, $6::jsonb)`,
        [createId(), sessionId, playerId, report.passed, report.badge, JSON.stringify(report)],
      );
  ...
  app.get<{ Params: { sessionId: string } }>(
    "/api/sessions/:sessionId/security-scans",
    async (request) => {
      const result = await query<{
        ...
        `SELECT player_id, passed, badge, report, created_at
