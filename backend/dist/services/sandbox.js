...
`;
function buildJsHarness(source, setup, expression) {
...
function runJsHarness(harness, timeoutMs) {
...
`;
function buildPythonHarness(source, setup, expression) {
    return [
...
function runPythonLocally(harness, timeoutMs) {
    const start = Date.now();
...
async function runHarnessViaPiston(language, harness, timeoutMs) {
...
async function runViaPiston(language, source, stdin, timeoutMs) {
    const { language: lang, version } = resolveLanguage(language);
    const url = `${config.pistonBaseUrl.replace(/\/+$/, "")}/execute`;
    const controller = new AbortController();
...
/* ────────────── Public API ────────────── */
export async function runCode(language, source, stdin = "", timeoutMs = 8_000) {
    return runViaPiston(language, source, stdin, timeoutMs);
}
function parseSandboxStdout(stdout) {
    const lines = stdout.split(/\r?\n/);
    for (let i = lines.length - 1; i >= 0; i--) {
        const line = lines[i];
        if (line.startsWith(RESULT_MARKER))
            return { result: line.slice(RESULT_MARKER.length) };
        if (line.startsWith(ERROR_MARKER))
            return { error: line.slice(ERROR_MARKER.length) };
    }
    return {};
}
...
async function runSingleTest(language, source, test, timeoutMs) {
...
export async function runChallengeTests(language, source, tests, timeoutMs = 5_000) {
    const out = [];
...
export async function runTests(language, source, tests) {
    const out = [];
    for (const test of tests) {
